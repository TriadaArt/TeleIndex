    ...
    <div>
      ...
      <div className="bg-white rounded-xl border p-4 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <button type="button" disabled={loading} className="tg-login" onClick={()=>submit('draft')}>Сохранить черновик</button>
          <button type="button" disabled={loading} className="tg-telega-reg" onClick={()=>submit('moderation')}>Отправить на модерацию</button>
          <button type="button" disabled={loading} className="tg-pill tg-pill-outline" onClick={()=>navigate('/me/channels')}>Отмена</button>
        </div>
      </div>
    </div>
  );
